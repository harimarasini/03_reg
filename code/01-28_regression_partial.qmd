...
# Model fitting
mod1_int <- lmer(yield_mgha ~ 1 + (1 | rep),      #random effect = rep, ~1 = linear model
                 data = reg_dfw
                 )
...
nd <- nd %>%
  mutate(mod1_yield_mgha = predict(mod1_int,
                                   nd,
                                   re.form = NA    # not to include random effect
                                   ))
